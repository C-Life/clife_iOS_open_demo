...
- (void)setupData
{
    ...
    DeviceDetailCellModel *deviceShare =  [DeviceDetailCellModel new];
    deviceShare.imageName = @"DeviceDetailsVC_deviceShare";
    deviceShare.titleName = ShareFuntionTitle ;
    
    DeviceDetailCellModel *scane =  [DeviceDetailCellModel new];
    scane.imageName = @"DeviceDetailsVC_deviceScan";
    scane.titleName = ScanQRCodeCellName ;
    
    [_dataSource addObject:scane];
    
    // 分享设备是没有下次分享权限
    if (_deviceModel.share.integerValue == 2) {
       [_dataSource addObject:deviceShare];
    }
