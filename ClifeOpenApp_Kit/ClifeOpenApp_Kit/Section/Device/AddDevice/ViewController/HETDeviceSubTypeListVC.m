...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    OPLog(@"dict = %@",dict);
    NSNumber *moduleTypeNumber = [dict objectForKey:@"moduleType"];
    NSInteger moduleType = moduleTypeNumber.integerValue;
    if (moduleType == HETBLEDeviceBindType) {

        NSString *deviceTypeStr=[NSString stringWithFormat:@"%@", [dict objectForKey:@"deviceTypeId"]];
        NSString *deviceSubTypeStr=[NSString stringWithFormat:@"%@", [dict objectForKey:@"deviceSubtypeId"]];
        NSString *productId=[NSString stringWithFormat:@"%@", [dict objectForKey:@"productId"]];

        HETBindBleDeviceViewController *moduleVC = [HETBindBleDeviceViewController new];
        moduleVC.deviceTypeStr=deviceTypeStr;
        moduleVC.deviceSubTypeStr=deviceSubTypeStr;
        moduleVC.productIdStr=productId;

        [self.navigationController pushViewController:moduleVC animated:YES];


    }else if(moduleType == HETWIFIDeviceBindType){
